...
/*
Thought about running (for and for loop n^2) 4 times for left & right, up & down, \ shape / shape
but figure it out make it at one for for loop
Not sure about memory access efficient, but its clearly not a good idea to access when array is big
70600674
*/
int main() {
  long ans = 0;
  for (int i = 0; i < 20; i++) {
    for (int j = 0; j < 20; j++) {
      ...
      // its tricky
      if (i > 2 && j > 2) {
        long tmp = grid[i][j-3] * grid[i-1][j-2] * grid[i-2][j-1] * grid[i-3][j];
        ans = ans > tmp ? ans : tmp; 
      }
    }
  }
